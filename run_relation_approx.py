...
from relation.relationModel import BertForRelationApprox

from transformers import AutoTokenizer
from transformers import AdamW, get_linear_schedule_with_warmup

from relation.relation_utils import generate_relation_data, decode_sample_id
...
task_ner_labels = {
    ...
    'faithlife': [
        'concept', 'writing', 'person', 'place', 'denom', 'denomgroup',
        'event', 'artifcat', 'thing'
    ]
}
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, position_ids, input_mask, labels,
                 sub_obj_ids, sub_obj_masks, meta, max_seq_length):
        # Padding
...
def get_features_from_file(filename,
        ...
        for i, sent in enumerate(doc):
            ...
            for x in range(len(sent_ner)):
                for y in range(len(sent_ner)):
                    ...
                    sub = sent_ner[x]
                    obj = sent_ner[y]
                    label = label2id[gold_rel.get((sub.span, obj.span),
                                                  'no_relation')]
                    SUBJECT_START_NER = get_special_token("SUBJ_START=%s" %
                                                          sub.label)
                    SUBJECT_END_NER = get_special_token("SUBJ_END=%s" %
                                                        sub.label)
                    OBJECT_START_NER = get_special_token("OBJ_START=%s" %
                                                         obj.label)
                    OBJECT_END_NER = get_special_token("OBJ_END=%s" %
                                                       obj.label)

                    if (len(tokens) + 4 >
                            max_seq_length) or (not (batch_computation) and
                    ...
                    tokens = tokens + [
                        SUBJECT_START_NER, SUBJECT_END_NER, OBJECT_START_NER,
                        OBJECT_END_NER
                    ]
                    position_ids = position_ids + [
            ...
            if num_shown_examples < 20:
                ...
                logger.info("input_ids: %s" %
                            " ".join([str(x) for x in features[-1].input_ids]))
                logger.info(
                    "position_ids: %s" %
                    " ".join([str(x) for x in features[-1].position_ids]))
                logger.info("input_mask: %s" % " ".join(
                    [str(x) for x in features[-1].original_input_mask]))
                logger.info("labels: %s" %
                            " ".join([str(x) for x in features[-1].labels]))
                logger.info("sub_obj_ids: %s" % " ".join([
                    '(%d, %d)' % (x[0], x[1]) for x in features[-1].sub_obj_ids
                ]))
                logger.info(
                    "sub_obj_masks: %s" %
                    " ".join([str(x) for x in features[-1].sub_obj_masks]))
                logger.info("sub_obj_spans: %s" % " ".join(
                    [str(x) for x in features[-1].meta['sub_obj_pairs']]))
    ...
    logger.info('# labels per sample on average: %f' %
                (num_label / len(features)))
...
def evaluate(model, device, eval_dataloader, e2e_ngold=None, verbose=True):
    ...
    if verbose:
        logger.info("***** Eval results (used time: %.3f s) *****" %
                    (time.time() - c_time))
        ...
        for pair in feat.meta['sub_obj_pairs']:
            ...
            if pred != 0:
                rels[doc_sent].append([
                    sub.start_doc, sub.end_doc, obj.start_doc, obj.end_doc,
                    id2label[pred]
                ])
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.output_dir, 'special_tokens.json')):
        with open(os.path.join(args.output_dir, 'special_tokens.json'),
                  'r') as f:
    ...
    if args.do_eval and (args.do_train or not (args.eval_test)):
        ...
        all_labels = torch.tensor([f.labels for f in eval_features],
                                  dtype=torch.long)
        all_sub_obj_ids = torch.tensor([f.sub_obj_ids for f in eval_features],
                                       dtype=torch.long)
        all_sub_obj_masks = torch.tensor(
            [f.sub_obj_masks for f in eval_features], dtype=torch.long)
        eval_data = TensorDataset(all_input_ids, all_position_ids,
                                  all_input_mask, all_segment_ids, all_labels,
    ...
    if args.do_train:
        ...
        all_labels = torch.tensor([f.labels for f in train_features],
                                  dtype=torch.long)
        all_sub_obj_ids = torch.tensor([f.sub_obj_ids for f in train_features],
                                       dtype=torch.long)
        all_sub_obj_masks = torch.tensor(
            [f.sub_obj_masks for f in train_features], dtype=torch.long)
        train_data = TensorDataset(all_input_ids, all_position_ids,
                                   all_input_mask, all_segment_ids, all_labels,
                                   all_sub_obj_ids, all_sub_obj_masks)
                ...
                if (step + 1) % eval_step == 0:
                    ...
                    if args.do_eval:
                        ...
                        if (best_result is
                            ...
                            save_trained_model(args.output_dir, model,
                                               tokenizer)
    ...
    if args.do_eval:
        if args.eval_test:
            eval_dataset, eval_features, eval_nrel = get_features_from_file(
                os.path.join(args.entity_output_dir,
        ...
        for key in sorted(result.keys()):
            logger.info("  %s = %s", key, str(result[key]))
        print_pred_json(eval_dataset, eval_features, preds, id2label,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--entity_predictions_dev",
                        type=str,
                        default="ent_pred_dev.json",
                        help="The entity prediction file of the dev set")
    parser.add_argument("--entity_predictions_test",
                        type=str,
    ...
    parser.add_argument('--task',
                        ...
                        default='faithlife',
                        required=False,
                        choices=['ace04', 'faithlife', 'ace05', 'scierc'])
